...
//	}
	...
	private void printLinhasLivro(String palavra, String filename, List<Integer> linhasEncontradas) {
		try {
			...
			String line = "";
			int lineNumber = 0; // controla o n�mero da linha
			int firstLine = linhasEncontradas.get(0);
			int lastLine = linhasEncontradas.get(linhasEncontradas.size()-1);
			while ((line = bfr.readLine()) != null) {
				line = this.normalizeLine(line); // usando a mesma normaliza��o da indexa��o
				
				//A primeira condi��o � tende a ser mais r�pida, eliminando a segunda quando n�o necess�rio. 
				...
				//Evita percorrer o livro at� o final, quando n�o necess�rio.
			...
			while ((line = bfr.readLine()) != null) {
				...
				for (String str : strs) {
					if (str.length() > 0 && str.length() < 30) {	
						// Coloca palavra no hash (se n�o existe) e retorna um id
	...
	private synchronized int getLivroId(String livro) {
		// N�o � necess�rio checar se j� exste, nunca vai repetir o mesmo livro
